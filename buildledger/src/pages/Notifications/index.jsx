        ...
        {filters.map(f => {
          ...
          return (
            <button key={f} onClick={() => setFilter(f)}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium transition-all ${filter === f ? 'bg-blue-600 text-white shadow-sm' : 'bg-white/60 text-slate-500 border border-white/80 hover:bg-white dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-700/40 dark:hover:bg-slate-700/60'}`}>
              {f}
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${filter === f ? 'bg-white/25 text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-700/60 dark:text-slate-400'}`}>{count}</span>
            </button>
          );
        })}
      </div>
        ...
        {['Contract','Delivery','Invoice','Compliance'].map(type => {
          ...
          return (
            <div key={type} className="glass-card p-4 flex items-center gap-3 cursor-pointer" onClick={() => setFilter(type)}>
              ...
              <div>
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">{type}</p>
                {count > 0
                  ? <p className="text-[10px] text-red-500 dark:text-red-400 font-semibold">{count} unread</p>
              ...
              <div key={n.id}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-xs leading-snug ${!n.read ? 'text-slate-800 dark:text-slate-100 font-semibold' : 'text-slate-600 dark:text-slate-400'}`}>{n.message}</p>
                    {!n.read && (
